...
<value>hdfs://'''+nnip+''':9001</value>
	...
	h.write(d)
	h.close()
	commands.getstatusoutput("docker exec {} hadoop-daemon.sh start datanode".format(dataid))
...
<value>'''+jtip+''':9002</value>
	...
	commands.getstatusoutput("docker exec {} hadoop-daemon.sh start tasktracker".format(dataid))
	print "Service Started at Container ID:\t"+dataid+"\n"
	print commands.getstatusoutput("docker exec {} /usr/java/jdk1.7.0_51/bin/jps".format(dataid))[1]
...
def status(nnid,jtid):
	print "\t\tHDFS STATUS\n"
	print commands.getstatusoutput("docker exec {} hadoop dfsadmin -report".format(nnid))[1]
	commands.getstatusoutput("docker exec {} hadoop dfsadmin -safemode leave".format(nnid))
	print "\nMAP REDUCE REPORT\n"
	print commands.getstatusoutput("docker exec {} hadoop job -list-active-trackers".format(jtid))[1]	
#######################################################################################################
...
jtid='ad435fe0988f'
#Namenode Start
commands.getstatusoutput("docker start fda35acf1362 ")
...
print commands.getstatusoutput("docker exec fda35acf1362  /usr/java/jdk1.7.0_51/bin/jps")[1]
#Jobtracker Start
...
for i in range(int(nodes)):
	j=i+90
	commands.getstatusoutput("docker run -itd -v /hdfsfiles:/etc/hadoop --privileged --name dn"+str(j) + " scpenablessh")[1]
dataid=commands.getstatusoutput("docker ps -a |grep scpenablessh |awk '{print $1}'")[1]
...
for i in  l:
	dn_auto(i,nnip)
	tt_auto(i,jtip)
commands.getstatusoutput("firefox 172.17.0.2:50070")
status(nnid,jtid)
